...
const PlanDetails = (props: PricingPlanProps) => {
  // ** Props
  const { billingPeriod, data, onPlanSelect } = props
  ...
  return (
    ...
    >
        ...
        <Box sx={{ my: 7, position: 'relative' }}>
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            ...
            <Typography variant='h3' sx={{ fontWeight: 600, color: 'primary.main', lineHeight: 1.17 }}>
              {billingPeriod === 'monthly' || !data?.yearlyPlan ? data?.monthlyPrice : data?.yearlyPlan.perMonth}
            </Typography>
            <Typography variant='body2' sx={{ mb: 1.6, fontWeight: 600, alignSelf: 'flex-end' }}>
              /month
            </Typography>
          </Box>
          {billingPeriod !== 'monthly' && data?.monthlyPrice !== 0 && data?.yearlyPlan ? (
            <Typography
              variant='caption'
      ...
      <Button
        ...
        disabled={data?.currentPlan || false}
        onClick={() => {
          if (data?.id && !data?.currentPlan) {
            onPlanSelect(data?.id)
          }
        }}
      >
        {data?.currentPlan ? 'Subscribed' : 'Upgrade'}
      </Button>
    </BoxWrapper>
  )
}
